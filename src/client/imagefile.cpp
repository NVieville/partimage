...
void CImage::set_szPath(const char * str)
{
  m_cid -> set_szPath(str);
}
void CImage::set_szImageFilename(const char * str)
{
  m_cid -> set_szImageFilename(str);
}
void CImage::set_szOriginalFilename(const char * str)
{
  m_cid -> set_szOriginalFilename(str);
}
void CImage::set_szImageFilename(const char * str, int n)
{
  m_cid -> set_szImageFilename(str, n);
}
void CImage::set_szOriginalFilename(const char * str, int n)
{
...
void CImage::writeSplit()
{
  ...
  showDebug(2, "image file split done\n");
...
void CImage::write(void *vBuf, DWORD dwLength, bool bUpdateCRC)
{
  ...
  qwCount += (QWORD) dwRes;
  if (qwCount > (1024 * 1024)) // update the window every 1 MiB
    qwCount = 0LL;
...
void CImage::writeMagic(const char *szMagicString)
{
  try { write((void *) szMagicString, strlen(szMagicString), true); }
  catch (CExceptions * execp)
    { throw execp; }
}
...
//           partitionvolume
void CImage::readAndCheckMagic(const char *szMagicString)
{
...
void CImage::openReading(CVolumeHeader *vh /* = NULL */)
{
  ...
  // ----------- check image version
  showDebug(4, "image file version: [%s] and CURRENT_IMAGE_FORMAT=[%s]\n", headVolume.szVersion, CURRENT_IMAGE_FORMAT);

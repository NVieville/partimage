...
{
  delete g_interface;
  g_interface = new CInterfaceNone((char *)"");
}
...
int main(int argc, char *argv[])
{
  ...
  strncpy(options.szFullyBatchMode, OPTIONS_DEFAULT_FULLYBATCHMODE, 2048);
...
//#ifndef DEVEL_SUPPORT
      else
	{
	  showDebug(1, "The file system of [%s] is [%s], and is not supported\n", szDevice, szFileSystem);
          ...
          g_interface->msgBoxError(i18n("The file system of [%s] is [%s], and is not supported"), szDevice, szFileSystem);
...
// if we reach this point, it's because shutdown failed
 ...
 // if we reach this point, it's because shutdown failed
	  ...
	  else
	    {
              --nValue;
	      fclose(fCountFile);
              fCountFile = fdopen(fd_countfile, "w");
	      if (fCountFile == NULL)
	        {
    	          delete g_interface; 
...
int checkStructSizes()
{
   ...
   if (sizeof(DWORD) != 4)
   {
      fprintf (stderr, i18n("Error: sizeof(DWORD) != 4 (%zd)\n"), sizeof(DWORD));
      goto errcheck;
   }

   if (sizeof(QWORD) != 8)
   {
      fprintf (stderr, i18n("Error: sizeof(QWORD) != 8 (%zd)\n"), sizeof(QWORD));
      goto errcheck;
   }
   ...
   {
      fprintf (stderr, i18n("Error: main header size != 16384 (%zd)\n"),
	       sizeof(CMainHeader));
      goto errcheck;
   }
   ...
   {	
      fprintf (stderr, i18n("Error: ext2 header size != %d (%zd)\n"), INFOS_STRUCT_SIZE,
   ...
   {	
      fprintf (stderr, i18n("Error: reiserfs header size != %d (%zd)\n"), INFOS_STRUCT_SIZE,
   ...
   {	
      fprintf (stderr, i18n("Error: fat header size != %d (%zd)\n"), INFOS_STRUCT_SIZE,
   ...
   {	
      fprintf (stderr, i18n("Error: ntfs header size != %d (%zd)\n"), INFOS_STRUCT_SIZE,
   ...
   {	
      fprintf (stderr, i18n("Error: hpfs header size != %d (%zd)\n"), INFOS_STRUCT_SIZE,
   ...
   {	
      fprintf (stderr, i18n("Error: jfs header size != %d (%zd)\n"), INFOS_STRUCT_SIZE,
   ...
   {	
      fprintf (stderr, i18n("Error: xfs header size != %d (%zd)\n"), INFOS_STRUCT_SIZE,
   ...
   {	
      fprintf (stderr, i18n("Error: hfs header size != %d (%zd)\n"), INFOS_STRUCT_SIZE,
   ...
   {	
      fprintf (stderr, i18n("Error: main footer size != 16384 (%zd)\n"),
		sizeof(CMainTail));
      goto errcheck;
   }
   ...
   {	
      fprintf (stderr, i18n("Error: volume header size != 512 (%zd)\n"),
   ...
   if (sizeof(CMbr) != 2048)
   {	
      fprintf (stderr, i18n("Error: MBR size != 2048 (%zd)\n"), sizeof(CMbr));
      goto errcheck;
   }
   ...
   {	
      fprintf (stderr, i18n("Error: local header size != 16384 (%zd)\n"),
     	 sizeof(CLocalHeader));
      goto errcheck;
   }
  ...
  errcheck:
   printf (i18n("This version has been compiled with an uncompatible version of gcc.\n"));

   ...
   COptions *options)
{
  ...
  // ----------- system checks
  nRes = systemChecks(szDevice, options->bFailIfMounted, options->szFullyBatchMode);
  if (nRes == -1)
    THROW( ERR_FSCHK);
  ...
  if (strlen(options->szFullyBatchMode)>0)
  {
    ...
    if (options->dwCompression == 0)
      ...
      fprintf(stderr,i18n("partimage: status: Image type: LZO\n"));
  }  
...
int detectFileSystem(const char *szDevice, char *szFileSys)
{
  FILE *fDeviceFile;
  int nRes;
  const int nBufSize = 128 * 1024; // 128 KiB
  ...
  // **** check for REISER3 file system [magic @ 65536]
  nRes = CReiser3Partdetect(cBuffer, szFileSys);
  if (nRes > 0)
    return nRes;

 // **** check for REISER4 file system [magic @ 65536]
  nRes = CReiser4Partdetect(cBuffer, szFileSys);
  if (nRes > 0)
    return nRes;
...
// =======================================================
int checkInodeForDevice(char *szDevice, char *szFullyBatchMode) // NEW FUNCTION
{
  int nRes;
  struct stat fStat;
  int nMajor, nMinor;
...
// =======================================================
int systemChecks(char *szDevice, bool bFailIfMounted, char *szFullyBatchMode)
{
...
int checkOptions(COptions options, char *szDevice, char *szImageFile)
{
  ...
  if ((options.qwSplitSize) && (options.qwSplitSize < (QWORD)(1024*1024)))
    {
      g_interface->msgBoxError(i18n("Split volumes size must be an integer greater than 1024 (size >= 1024 KiB)"));	
      RETURN_int(-1);
    }
...
// =======================================================
int CReiser3Partdetect(BYTE *cBootSect, char *szFileSystem)
{
  BEGIN;
  
/*  static int dv=0;
char bn[1024];
sprintf(bn,"BOOTIN_%d",dv); dv++;
FILE* fpp=fopen(bn,"wb");
fwrite(cBootSect,128000,1,fpp);
fclose(fpp);
*/
...
// =======================================================
int CReiser4Partdetect(u8 *cBootSect, char *szFileSystem)
{
  BEGIN;
  
  if (memcmp(cBootSect+REISER4FS_DISK_OFFSET_IN_BYTES, REISER4FS_SUPER_MAGIC_STRING, strlen(REISER4FS_SUPER_MAGIC_STRING)) == 0)
    {
      strcpy(szFileSystem, "reiserfs-4.0");
      return FS_REISER_4_0;
    }
  
  showDebug(4, "001 = after check for Reiser-4.0\n");
...
int CUfsPartdetect(BYTE *cBootSect, char *szFileSystem, FILE *fDeviceFile)
{
  ...
  int nRes;
  off_t off;
  u_int dev_bsize = 512; /* XXX check block size in the disk label */
  ...
  for (i=0; i < (DWORD)sbUfs.fs_ncg; i++)
    {
      ...
      off = (off_t)fsbtodb(&sbUfs, cgtod(&sbUfs, i)) * dev_bsize;
      nRes = bigFseek(fDeviceFile, off);
      if (nRes == -1)
	{

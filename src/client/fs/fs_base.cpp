...
void CFSBase::saveToImage(CImage *image, COptions *options, CSavingWindow *gui)
{
  ...
  if (strlen(options->szFullyBatchMode)>0)
      fprintf(stderr,i18n("File Name    Size     T:Elapsed/Estimated  Rate/min     Progress\n"));
...
void CFSBase::restoreFromImage(CImage *image, COptions *options, CRestoringWindow *gui)
{
  ...
  // 1. ---- Read header
  g_interface -> StatusLine(i18n("reading header"));
  ...
  g_interface -> StatusLine(i18n("copying used data blocks"));
  if(strlen(options->szFullyBatchMode)>0)
     fprintf(stderr,i18n("File name    Level    T:Elapsed/Estimated  Rate/min     Progress\n"));

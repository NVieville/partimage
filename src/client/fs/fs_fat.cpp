...
{	
  ...
  //int nResult; // Commented for GCC warnings
  int nRes;
  ...
  // init
  //nResult = 0; // Commented for GCC warnings
...
void CFatPart::readSuperBlock()
{
  ...
  if (m_info.cSectorsPerCluster == 0)
    {
      g_interface->msgBoxError(i18n("The sectors per cluster number can't be null. Check the partition to verify it's a FAT one.") );
      THROW(ERR_WRONG_FS);
    }
  ...
  if ((nRes == -1) && (m_info.dwFileSystem == FS_FAT32))
    {
      showDebug(1, "Can't read the filesystem information sector\n");
      g_interface->msgBoxError(i18n("Can't read the filesystem information sector"));
      setSuperBlockInfos(false, false, 0, 0);
    }

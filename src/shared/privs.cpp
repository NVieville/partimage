...
// we can't use showDebug since not yet opened
  ...
  else
    {
      ...
      if (password)
        ...
        {
// we can't use showDebug since not yet opened
          fprintf(stderr, i18n("Changing to user '%s' failed:\n"), _user);
          fprintf(stderr, i18n("impossible to get user '%s' from /etc/passwd.\n"),
             _user);
          fprintf(stderr, i18n("You must add '%s' to your password file.\n"), _user);
          fprintf(stderr, i18n("See: 'man adduser' or 'man useradd' for help.\n"));
          exit(1);
        }
    }
}

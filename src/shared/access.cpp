/***************************************************************************
                          access.cpp  -  allow or not connection from user
                             -------------------
...
#ifndef HAVE_PAM
static void help(const char * szFile)
{
  fprintf(stderr,i18n("error when verifying %s, check logfile for help\n"),
     szFile);
  fprintf(stderr, i18n("logfile is %s\n"), PARTIMAGED_LOG);
  fprintf(stderr, i18n("if you don't have logfile, use partimaged --debug=1\n"));
}

unsigned int CheckAccessFile(const char * szFile)
{
  int nRes;
  struct stat status;
  mode_t mode;

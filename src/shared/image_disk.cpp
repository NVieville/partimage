...
#define SPACEFILE_SIZE (10*1024*1024) // 10 MiB
	  ...
	  if (dwRes != dwLength-dwFirstWritten) // out of space
	    {
	      *nResult = ERR_INCOMPLETEFINISH;
	      ...
	      newtWinMessage((char *)"error", (char *)"ok", (char *)"ERROR: ERR_INCOMPLETEFINISH (dwRes != dwLength-dwFirstWritten) "
...
void CImageDisk::getDiskFreeSpaceForFile(QWORD *qwAvailDiskSpace, char *szFilepath) // [Main-Thread]
{
  #warning "getDiskFreeSpaceForFile always return 5 MiB"

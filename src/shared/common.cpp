...
char *formatTime2(DWORD dwSec, char *szBuf, int nMaxLen)
{
  ...
  *szBuf = 0;
  if (nHour)
    snprintf(szBuf, nMaxLen, "%2dh:", nHour);
  if (nMin || nHour)
    snprintf(szBuf+strlen(szBuf), nMaxLen-strlen(szBuf), "%2dm:", nMin);
  snprintf(szBuf+strlen(szBuf), nMaxLen-strlen(szBuf), "%2dsec", nSec);
...
char *formatSizeNoGui(QWORD qwSize, char *szText, int nMaxLen)
{
  ...
  if (qwSize < llKiloB) // In Bytes
    ...
    {
      dSize = ((double) qwSize) / ((double) 1024.0);
      snprintf(szText, nMaxLen, i18n("%4.0f KiB"), (float) dSize);
    }
  else if (qwSize < llGigaB) // In MegaBytes
    {
      dSize = ((double) qwSize) / ((double) (1024.0 * 1024.0));
      snprintf(szText, nMaxLen, i18n("%4.0f MiB"), (float) dSize);
    }
  else if (qwSize < llTeraB)// In GigaBytes
    {
      dSize = ((double) qwSize) / ((double) (1024.0 * 1024.0 * 1024.0));
      snprintf(szText, nMaxLen, i18n("%1.2f GiB"), (float) dSize);
    }
  else // In TeraBytes
    {
      dSize = ((double) qwSize)/((double) (1024.0 * 1024.0 * 1024.0 * 1024.0));
      snprintf(szText, nMaxLen, i18n("%1.2f TiB"), (float) dSize);
    }
...
FILE *openFileDescriptorSecure(const char *szFilename, const char *szMode, int nFlags, mode_t mode)
{
  int nFd;
  FILE *fStream;
#ifdef HAVE_FSTAT64
